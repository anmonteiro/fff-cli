...
PROMPT_TEXT="~/monorepo/dotfiles/nvim/fff-tui % "
...
wait "$cap_pid"
ffmpeg -y -framerate 10 -i "$tmp_frames/frame_%04d.png" -vf format=yuv420p "$OUT" >/tmp/fff-history-demo-kitty.log 2>&1
ffmpeg -y -ss 9 -i "$OUT" -frames:v 1 "$FRAME" >/tmp/fff-history-demo-kitty-frame.log 2>&1
rm -rf "$tmp_frames"
